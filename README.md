...
처음 이 모델을 실제 프로젝트에 붙일 때는 아래 순서로 보면 됩니다.

1. Prime를 누구로 둘지 정한다
2. 분신 역할을 3~5개로 최소화한다
3. 각 분신에 대응하는 실제 worktree/branch/path를 만든다
4. 분신별 시작 전 상태 점검 루틴을 고정한다
5. Prime 검토 게이트와 사용자 보고 포맷을 통일한다
6. 24/7 루프라면 결과 로그와 학습 로그 저장 위치를 먼저 정한다

빠르게 말하면:
- 역할명부터 만들지 말고 실제 경로부터 만든다
- 완료 표현은 Prime만 쓴다
- 자율실행은 작업량이 아니라 결과 변화로 관리한다

## 자주 헷갈리는 반패턴

이 레포가 특히 막고 싶은 반패턴은 아래입니다.

- 같은 working tree를 여러 분신이 번갈아 쓰면서 병렬이라고 부르는 것
- 분신이 직접 사용자에게 완료를 말하는 것
- 테스트 통과를 곧바로 배포 성공으로 착각하는 것
- 분신별 역할은 나눴는데 상태 확인 루틴은 없는 것
- 자율루프가 변경 이력만 남기고, 실제 결과/효과는 기록하지 않는 것

이런 경우는 멀티에이전트가 아니라 단일 작업 공간의 혼선에 가깝습니다.
...
- `docs/report-templates.md`
  - 분신 보고 / Prime 최종 보고 / 24시간 루프 기록 템플릿

- `docs/anti-patterns.md`
  - 말뿐인 분신 운영에서 자주 생기는 실패 패턴 정리
